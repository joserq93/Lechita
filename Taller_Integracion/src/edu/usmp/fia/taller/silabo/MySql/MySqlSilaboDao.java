...
public class MySqlSilaboDao extends MySqlDAOFactory implements DAOSilabo{
	...
	public Vector<CursoBean> listarCursoxSilabo() throws Exception {
		...
		try {
			...
			String query = "SELECT * FROM t_curso c,t_plan_curricular_detalle pc where c.estado_silabo='1' and c.id=pc.curso_id";
			ResultSet rs = stmt.executeQuery(query);
			VECTOR_OUT = new Vector<CursoBean>();
			while(rs.next()){			
				CursoBean OBJETO = new CursoBean();
	...
	public Vector<SilaboBean> listarSilabo(String id) throws Exception {
		...
		try {
			...
			while(rs.next()){			
				...
				OBJETO.setRequisito(rs.getString("pc.creditos_requisito"));
	...
	public boolean modificarSemana(String id,String tema) throws Exception {
		try {
			Connection con = MySqlDAOFactory.obtenerConexion();
			Statement stmt = con.createStatement();
			stmt.executeUpdate("update t_semana set reseña='"+tema+"' where id_semana="+id+"");
		} catch (Exception e) {
			System.out.print(e.getMessage());
		}
		return false;
	...
	public Vector<CursoBean> listarCurso() throws Exception {
		...
		try {
			...
			String query = "SELECT * FROM t_curso order by nombre asc;";
			ResultSet rs = stmt.executeQuery(query);
			VECTOR_OUT = new Vector<CursoBean>();
			while(rs.next()){			
				CursoBean OBJETO = new CursoBean();
	...
	public boolean eliminarCurso(String id)throws Exception {
		...
		try {
			Connection con = MySqlDAOFactory.obtenerConexion();
			Statement stmt = con.createStatement();
			String query = "DELETE FROM t_curso WHERE id='"+id+"'";
	...
	public boolean modificarCurso(String idN,String id,String nombre,String tipo, String sumilla)throws Exception {
		try {
			Connection con = MySqlDAOFactory.obtenerConexion();
			Statement stmt = con.createStatement();
			int t=Integer.parseInt(tipo);
			stmt.executeUpdate("update t_curso set id='"+idN+"', nombre='"+nombre+"',sumilla='"+sumilla+"',tipo="+t+" where id='"+id+"'");
		} catch (Exception e) {
			System.out.print(e.getMessage());
		}
		return false;
	...
	public boolean validarCurso(String nombre,String codigo)throws Exception {
		boolean flag=false;
		try {
			Connection con = MySqlDAOFactory.obtenerConexion();
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("Select * from t_curso where nombre='"+nombre+"' OR id='"+codigo+"'");
			if(rs.next()){
				flag=false;
			}else{
				flag=true;
	...
	public boolean validarCursoM(String nombre,String codigo)throws Exception {
		boolean flag=false;
		try {
			Connection con = MySqlDAOFactory.obtenerConexion();
			Statement stmt = con.createStatement();
