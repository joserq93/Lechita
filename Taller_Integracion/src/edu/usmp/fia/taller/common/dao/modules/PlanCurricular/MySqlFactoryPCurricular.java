...
public class MySqlFactoryPCurricular extends MySqlDAOFactory implements DAOFactoryPCurricular {
	...
	public List<T_Curso> obtenerT_Curso() {
		...
		try {
			...
			String query4 = "select * from t_curso order by nombre";
	...
	public String obtenerNombre(String code){
		...
		try {
			...
			String query = "select nombre from t_curso where id like '%" + code + "%'";
	...
	public List<String> guardarCursos(List<Curso> cursosCambiados) {	
		...
		try {
			...
			for (int i = 0; i < cursosCambiados.size(); i++) {
				...
				String query = "";
				if (tmp.getCreditosReq() == 0){
					query = "INSERT INTO `t_plan_curricular_detalle` "
							+ "(`id`, `curso_id`, `creditos`, `ciclo_id`, `curso_condicion_id`, `horasTeoria`, "
							+ "`horasLaboratorio`, `horasPractica`, `curso_area_id`, `Ordenar`, estado) "
							+ "VALUES (1, '" + tmp.getCode() + "', " + tmp.getCredits() + ", " + tmp.getCycle() + ", "
							+ "" + tmp.getType() + ", " + tmp.getTheoHours() + ", " + tmp.getLaboHours() +", "
							+ "" + tmp.getPracHours() + ", " + tmp.getCursoArea() + ", "
							+ "" + tmp.getOrder() + ", ' ')";
				} else {
					query = "INSERT INTO `t_plan_curricular_detalle` "
							+ "(`id`, `curso_id`, `creditos`, `ciclo_id`, `curso_condicion_id`, `horasTeoria`, "
	...
	public List<String> actualizarCursos(List<Curso> cursosActualizados) {		
		List<String> mensajes = new ArrayList<String>();
		String mensajeTPC = null;
		String mensajeTPR = null;
		String mensajeTPM = null;
		System.out.println("MySqlFactoryPCurricular.actualizarCursos()");
		try {
			Connection con = MySqlDAOFactory.obtenerConexion();
			Statement stmt = con.createStatement();
			Curso tmp = new Curso();
			...
			for (int i = 0; i < cursosActualizados.size(); i++) {
				System.out.println("si hay cursos actualizados");
				...
				for (int j = 0; j < requisitos.size(); j++) {
					...
					if (filas2 == 1) {
						mensajeTPR = "Se ejecturaon los cambios en los requisitos del curso";
						mensajes.add(mensajeTPR);
					}
				}
				...
				for (int k = 0; k < menciones.size(); k++) {
					...
					if (filas3 == 1) {
						mensajeTPM = "Se ejecturaon los cambios en la mencion de curso";
						mensajes.add(mensajeTPM);
					}
				}
			}
			return mensajes;
